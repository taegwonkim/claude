...
/* PC 프레임(§1)의 DC IP/MAC 필드용 캐시. Esp32_Poll()이 WiFi 연결 성공 시 갱신하고,
 * FpgaLink_Poll() 등은 Esp32_GetCachedNetInfo()로만 읽는다. RTOS가 없어 모든 호출이
 * 같은 super-loop 스레드에서 순차 실행되므로 락은 필요 없다. */
static char s_cachedIp[16] = "0.0.0.0";
static char s_cachedMac[18] = "00:00:00:00:00:00";

/* AT 드라이버 함수들은 main()의 super-loop 단일 스레드에서만 호출됨을 전제로 한다. */
 ...
 * RTOS가 없으므로 이 함수는 최대 timeout_ms 동안 super-loop 전체를 블로킹한다
