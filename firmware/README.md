# STM32L562C + FreeRTOS — WiFi 계측 브릿지 (Firmware)

STM32L562C(Cortex-M33) 위에서 FreeRTOS(CMSIS-RTOS2)를 사용해, PC ↔ MCU ↔ ESP32(WiFi) ↔ PC서버,
그리고 FPGA → MCU(트리거+ADC값) 경로를 처리하는 펌웨어 애플리케이션 코드입니다. 이 저장소는
firmware(이 디렉터리)와 [PC용 C# 도구](../pc-app/)로 나뉘며, 둘 사이의 통신 규격은
[`../docs/프로토콜_명세.md`](../docs/프로토콜_명세.md)를 공통 기준으로 삼습니다.

이 디렉터리는 **STM32CubeMX가 생성하는 프로젝트에 얹는 애플리케이션 레이어**입니다.
...
firmware/
  ...
  Core/Inc/, Core/Src/  - 아래 참고
../docs/
  프로토콜_명세.md        - PC↔MCU STX+CSV+CRLF 프레임 커맨드셋, 플래시 메모리 맵, ESP32 AT 시퀀스,
                          FPGA 프레임 포맷 (firmware/pc-app 공통 기준, 저장소 루트에 위치)
../pc-app/               - PC용 C# 설정/모니터링 도구 (별도 README 참고)
