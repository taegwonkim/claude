...
매핑합니다. 이 프로젝트는 3개의 애플리케이션 태스크를 사용합니다.

| 태스크        | 우선순위 (`osPriority_t`)   | 근거                                                                 |
|----------------|------------------------------|-----------------------------------------------------------------------|
| `fpgaIfTask`   | `osPriorityAboveNormal1`     | Cyclone IV 트리거(EXTI)에 즉시 반응해 USART3 DMA 수신을 준비해야 함. 세 태스크 중 가장 실시간성이 높음. 지연되면 다음 트리거 전까지 데이터 유실 가능 |
| `esp32Task`    | `osPriorityAboveNormal`      | AT 명령 송수신은 블로킹으로 수백ms~수초 걸릴 수 있어 fpgaIfTask보다는 낮지만, PC 설정 처리보다는 응답성이 중요 (측정 데이터 전송 경로) |
| `pcUartTask`   | `osPriorityNormal`           | 사람이 입력/스크립트로 보내는 설정 명령 처리. 지연에 가장 관대함 |
...
**규칙**: `fpgaIfTask > esp32Task > pcUartTask`. `configUSE_PREEMPTION`은 반드시
`1`이어야 트리거 발생 시 낮은 우선순위 태스크를 즉시 선점할 수 있습니다.
...
|----------------|-----------:|-----------:|------|
| `pcUartTask`   | 384        | 1536 B     | 짧은 라인 파서 + `snprintf` 응답 버퍼(64B) 정도만 사용 |
...
| TCB(태스크 제어 블록) 5개              | ~750 B    |
| 큐 1개(`esp32ReqQueue`, capacity 4)    | ~150 B    |
| 뮤텍스/세마포어/이벤트그룹 (5~6개)     | ~500 B    |
...
합계 약 2.2KB — 무시해도 될 수준입니다. 측정 데이터량이 늘어나면
...
- **Tasks and Queues** 탭
  - CubeMX가 기본 생성하는 `defaultTask`는 **삭제**하세요. 이 프로젝트는
    `App_CreateTasks()`(`app_tasks.c`)가 `osThreadNew()`로 3개 태스크를 직접
    생성하므로 `MX_FREERTOS_Init()`에는 커스텀 태스크를 추가하지 않습니다.
