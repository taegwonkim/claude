...
| **전원 off/on 영향** | 없음 (그 시점부터 재계수) | **큼** (시각을 다시 맞춰야 함) |
...
> **VBAT 에 별도 배터리가 없다면(= VBAT 를 VDD 에 연결) ② 는 신중히 고르세요.**
> 전원을 내릴 때마다 RTC 달력이 지워져서, 전원 인가 때마다 시각을 다시
> 넣어주지 않으면 "매일 03:00" 이 엉뚱한 시각에 발생합니다.
> 자세한 내용은 각 프로젝트 README 의 **1-3절**을 보세요.
...
- **백업 전원 전제 : VBAT = VDD** (별도 배터리 없음)
  - 소프트웨어 리셋 / NRST 리셋 → RTC·백업 레지스터 **유지**
  - 전원 off → on → **초기화** (콜드 부트로 판별해 로그에 표시)
- **TAMP 백업 레지스터**에 "이번 전원 인가 이후" 소프트 리셋 횟수 저장
- LSE 기동 실패 시 **LSI 로 폴백** (전원 인가 때마다 발진기를 새로 켜야 하므로,
  기동 실패로 보드가 멈추지 않도록)
