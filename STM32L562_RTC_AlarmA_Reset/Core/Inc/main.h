...
/* ===== 부팅 시 UART 시각 동기화 (선택) ==================================
 * 이 보드는 VBAT 가 MCU 전원(VDD)에 연결되어 있어, 전원을 내리면 RTC 달력이
 * 지워진다. Alarm 은 "벽시계 시각" 기준이므로 전원을 넣을 때마다 실제 시각을
 * 넣어주지 않으면 "매일 03:00" 이 엉뚱한 시각에 발생하게 된다.
 *   1U : 콜드 부트 시 UART 로 "YYYY-MM-DD HH:MM:SS" 를 받아 RTC 달력에 설정
 *        (TIME_SYNC_TIMEOUT_MS 안에 입력이 없으면 기본값으로 진행)
 *   0U : 사용 안 함 (RTC_INIT_FROM_BUILD_TIME 값으로 시작)
 * USE_DEBUG_UART 가 1 일 때만 동작한다.
 * ===================================================================== */
#define USE_UART_TIME_SYNC     0U
#define TIME_SYNC_TIMEOUT_MS   5000U
